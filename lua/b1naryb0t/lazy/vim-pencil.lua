  ...
  config = function()
    ...
    vim.api.nvim_create_autocmd("FileType", {
      ...
      callback = function()
        ...
        -- Scoped to writing buffers only so they don't affect code files
        vim.keymap.set("n", "j", "gj", { buffer = true, desc = "Move down by visual line" })
        vim.keymap.set("n", "k", "gk", { buffer = true, desc = "Move up by visual line" })
        vim.keymap.set("n", "0", "g0", { buffer = true, desc = "Go to beginning of visual line" })
        vim.keymap.set("n", "$", "g$", { buffer = true, desc = "Go to end of visual line" })
        vim.keymap.set("i", "<C-e>", "<C-o>g$", { buffer = true, desc = "Go to end of visual line" })
      end,
    })
    ...
    -- Keymaps for pencil functionality (global, not buffer-local)
